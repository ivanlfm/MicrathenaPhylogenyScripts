...
#########################################
	...
	#analysis name to be appended to output files
		filename <- "Micrathena_all_6areas"
	#name of the output directory	
		dir_name <- "BGB_multiple_trees_nomorph"
...
########################################################
	...
	### TREES
		#file with the posterior trees. This script assumes you have already discarded the pre-burn-in trees
			#you can use the output of BEAST's LogCombiner directly
				posterior_trees_fn <- "1_log12_combined_trees.nex"
	...
	### DISTRIBUTIONS
		# name of the file containing the distributions
			geogfn = "1_onlyDNA_species_6.txt"
	...
	### BIOGEOBEARS OPTIONS
		...
		#set these to TRUE and indicate the filenames for using dispersal matrices, time slices, etc.
			...
			dispersal_matrix_fn = "2_dispersalmatrix_adjacency.txt"
...
##################################
	...
	# gets the number of edges
		...
		tr <- read.tree(trfn)
		pruned_trees <- keep.tip(tr, tip=species)
		ingroup <- drop.tip(pruned_trees, tip = outgroup_taxa)
	...
	# only proceeds with analysis if everything is OK
		...
		for (r in 1:n_trees) {
			...
			# reads one of the trees of the posterior distribution and loads it to BGB object
				...
				tr <- read.tree(trfn)
				pruned_tree <- keep.tip(tr, tip=species)
				write.tree(pruned_tree, file = paste(dir_name, "/trees/tree",r,"_pruned.nex", sep= ""))
				ingroup <- drop.tip(pruned_tree, tip = outgroup_taxa)
				trfn <- paste(dir_name, "/trees/tree",r,"_pruned.nex", sep= "")
				tr <- read.tree(trfn)
